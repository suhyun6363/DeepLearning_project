...
def create_model(filters1, filters2, filters3, kernel_size, padding, pool_size, optimizer, learn_rate,
                 init_mode_1, init_mode_2, init_mode_3):
    model = Sequential()
    model.add(Conv2D(filters1, kernel_size, padding=padding, kernel_initializer=init_mode_1, activation='relu', input_shape=(28, 28, 1)))
    model.add(Conv2D(filters2, kernel_size, padding=padding, kernel_initializer=init_mode_2, activation='relu'))
    model.add(MaxPooling2D(pool_size))
    model.add(Dropout(0.25))
    model.add(Flatten())
	model.add(Dense(filters3, kernel_initializer=init_mode_3, activation='relu'))
